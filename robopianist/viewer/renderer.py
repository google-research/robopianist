...
class OffScreenRenderer(BaseRenderer):
    ...
    def __init__(self, model, surface, font_scale):
        """Instance initializer.
    ...
    def _render_on_gl_thread(self, viewport, scene):
        mujoco.mjr_setBuffer(
            mujoco.mjtFramebuffer.mjFB_OFFSCREEN, self._mujoco_context.ptr
        )
        ...
        mujoco.mjr_readPixels(
            self._rgb_buffer, None, viewport.mujoco_rect, self._mujoco_context.ptr
        )
...
class Perturbation:
    ...
    def __init__(self, body_id, model, data, scene):
        ...
        mujoco.mjv_initPerturb(
            self._model.ptr, self._data.ptr, self._scene.ptr, self._perturb.ptr
        )
    ...
    def apply(self, paused):
        ...
        mujoco.mjv_applyPerturbPose(
            self._model.ptr, self._data.ptr, self._perturb.ptr, int(paused)
        )
        if not paused:
            mujoco.mjv_applyPerturbForce(
                self._model.ptr, self._data.ptr, self._perturb.ptr
            )
        yield
...
class RenderSettings:
    ...
    def toggle_visualization_flag(self, flag_index):
        ...
        self._visualization_options.flags[
            flag_index
    ...
    def toggle_geom_group(self, group_index):
        ...
        self._visualization_options.geomgroup[
            group_index
    ...
    def toggle_site_group(self, group_index):
        ...
        self._visualization_options.sitegroup[
            group_index
    ...
    def select_next_rendering_mode(self):
        ...
        self._visualization_options.frame = (
            self._visualization_options.frame + 1
        ) % mujoco.mjtFrame.mjNFRAME
    ...
    def select_next_labeling_mode(self):
        ...
        self._visualization_options.label = (
            self._visualization_options.label + 1
        ) % mujoco.mjtLabel.mjNLABEL
...
class SceneCamera:
    ...
    def zoom_to_scene(self):
        ...
        self.look_at(
            self._model.stat.center[:], self._zoom_factor * self._model.stat.extent
        )
...
class Viewport:
    ...
    def screen_to_inverse_viewport(self, screen_coordinates):
        ...
        """
        x = screen_coordinates[0] / self._screen_size.width
        y = 1.0 - (screen_coordinates[1] / self._screen_size.height)
        return np.array([x, y], np.float32)
