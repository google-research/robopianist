...
from robopianist.viewer.gui import base, fullscreen_quad
...
class GlfwMouse(base.InputEventsProcessor):
    ...
    def __init__(self, context):
        ...
        with context.make_current() as ctx:
            framebuffer_width, window_width = ctx.call(self._glfw_setup, context.window)
...
class GlfwWindow:
    ...
    def window_shape(self):
        ...
        with self._context.make_current() as ctx:
            return ctx.call(glfw.get_window_size, self._context.window)
    ...
    def pixels_per_inch(self) -> float:
        with self._context.make_current() as ctx:
            monitor = ctx.call(glfw.get_primary_monitor)
            video_mode = ctx.call(glfw.get_video_mode, monitor)
            size_mm = ctx.call(glfw.get_monitor_physical_size, monitor)
            return 25.4 * video_mode.size.width / size_mm[0]
